...
void Pipeline::InitRotateTransform(Matrix4f& m) const
{
    ...
    const float x = DegreesToRadians(m_rotateInfo.x);
    const float y = DegreesToRadians(m_rotateInfo.y);
    const float z = DegreesToRadians(m_rotateInfo.z);
...
void Pipeline::InitPerspectiveProj(Matrix4f& m) const
{
    ...
    const float tanHalfFOV = tanf(DegreesToRadians(m_persProj.FOV / 2.0f));
